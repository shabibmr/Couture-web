...
import { Banner } from '@/types'; // Adjust import path if needed
...
const Hero: React.FC = () => {
    ...
    React.useEffect(() => {
        logger.info('Component Mounted: Hero');
        ...
        <section ref={heroRef} className="relative min-h-screen grid grid-cols-1 md:grid-cols-2 pt-20 md:pt-0">
            ...
            <div className="relative z-20 flex flex-col justify-center px-6 md:px-20 py-20 bg-beige-bg order-2 md:order-2">
                ...
                >
                    ...
                    <div className="pt-8">
                        <Link href={link} className="group inline-flex items-center gap-4 text-sm tracking-widest uppercase text-stone-800 hover:text-ruvera-gold transition-colors">
            ...
            >
                {/* 
                   Using standard img tag here because simpler for dynamic/external images 
                   checking valid domain in next.config might be tricky for arbitrary banner URLs.
                   If bgImage is local, we could use Image. For now, matching standard React behavior
                   but wrapping in picture struct.
                */}
